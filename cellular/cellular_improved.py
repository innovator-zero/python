...
def tmp_membership(x,a):
    ...
    if fit_tmp < x and x < fit_tmp + DLT_TMP:
        return (x- fit_tmp)/DLT_TMP
    elif  fit_tmp - DLT_TMP< x and x <= fit_tmp:
        return (fit_tmp - x)/DLT_TMP
    else:
        return 0
...
#                 temp_data[p][q] += 0.12
    for i_ter in range(2):
        buf = np.zeros(herring.shape)
        for i in range(5,348):
            for j in range(2,782):
                if not np.isnan(herring[i][j]):
                    # ram_mig_1, ram_mig_2
                    neighbor = herring[i-3:i+2,j-2:j+3]
                    temp_neighbor = temp_data[i-3:i+2,j-2:j+3]
                    ...
                    a = np.random.randn(1)[0]/3
                    for p in range(5):
                        for q in range(5):
                            move_pro[p][q] = membership(temp_neighbor[p][q]+0.07*year,neighbor[p][q],a)
                    move_pro = move_pro/np.nansum(move_pro)
                    move_num = ram_mig_1(move_pro,int(herring[i][j]))
                    for p in range(5):
                        for q in range(5):
                            if not np.isnan(herring[i-3+p][j-2+q]):
                                buf[i-3+p][j-2+q] += move_num[p][q]
        ...
        plt.imshow(herring,cmap=plt.cm.get_cmap('tab20c'))
        plt.savefig('pictures2/herring_{}_{}.png'.format(year+2021,i_ter))
        plt.pause(1)
        np.savetxt('data/herring_{}_{}.txt'.format(year+2021,i_ter),herring)
