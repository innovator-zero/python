...
def tmp_membership(x,a):
    ...
    if fit_tmp < x and x < fit_tmp + DLT_TMP:
        return 1-(x- fit_tmp)/DLT_TMP
    elif  fit_tmp - DLT_TMP< x and x <= fit_tmp:
        return 1-(fit_tmp - x)/DLT_TMP
    else:
        return 0
...
#                 temp_data[p][q] += 0.12
    for i_ter in range(2):
        ...
        for i in range(2,348):
            for j in range(2,782):
                if not np.isnan(herring[i][j]):
                    ...
                    for p in range(5):
                        for q in range(5):
                            move_pro[p][q] = membership(temp_neighbor[p][q]+0.085*year,neighbor[p][q],a)
        ...
        plt.imshow(herring,cmap=plt.cm.get_cmap('tab20c'))
        plt.savefig('pictures_0.085/herring_{}_{}.png'.format(year+2021,i_ter))
        plt.pause(1)
        np.savetxt('data_0.085/herring_{}_{}.txt'.format(year+2021,i_ter),herring)
